function ekgSrcFinal = reviewEkgArtifactUI_tiled(W, xICA, fs, chanlocs, ekgSrc, candidates, StartSec, NSec, figName)
% reviewEkgArtifactUI_tiled
...
% Auto-centers and clamps figure position to stay on-screen (no pages, no scrollbars).

if nargin < 10 || isempty(figName), figName = 'Review EKG Artifacts (tiled)'; end
...
figH = ctrlH + (nRows*rowH_est + 60);  % let it be tall if needed (you said height isn't the issue)
...
% Re-clamp if MATLAB resizes the window (e.g., titles/fonts/layout settle)
...
% Suppress specific tiledlayout position warning (some topo funcs touch Position)
warning('off','all');
